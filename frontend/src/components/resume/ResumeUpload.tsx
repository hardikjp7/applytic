...
interface UploadedResume { versionName: string; filename: string; uploadedAt: string }
...
export default function ResumeUpload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploaded(prev => [...prev, { versionName, filename: file.name, uploadedAt: new Date().toLocaleDateString() }])
      toast.success(`Resume "${versionName}" uploaded`)
      setFile(null); setVersionName('')
  ...
  return (
    <div className="p-4 lg:p-6 max-w-2xl">
      ...
      {uploaded.length > 0 && (
        <div className="mt-6">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Uploaded this session</p>
